...
LRESULT CMainDlg::OnInitDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    ...
    if(SUCCEEDED(hr))
    {
        for(LONG i = 0; i < lLibCount; i++)
        {
            ...
            if(SUCCEEDED(hr))
            {
                SendMessage(hLibList, CB_ADDSTRING, 0, (LPARAM)OLE2W(bstrLibName));
            }
        }
    }
        ...
        if(hParentNode && m_pnpNodeParams[iSchemaIndex].szAttrName)
        {
            ...
            if(SUCCEEDED(hr))
            {
                ...
                for(LONG i = 0; i < lCount; i++)
                {
                    ...
                    if(SUCCEEDED(hr))
                    {
                        tvins.item.pszText = bstrVal.Length() ? OLE2W(bstrVal) : g_szUnknown;
    ...
    if(m_spMC)
    {
        ...
        if(spSC)
        {
            ...
            for(LONG i = 0; i < lCount; i++)
            {
                ...
                if(bstrVal.m_str)
                {
                    SendMessage(m_hDetailList, LB_ADDSTRING, 0, (LPARAM)OLE2W(bstrVal));
                }
            }
        }
    }
