...
[!endif]
[!if HASPROPERTYPAGE]
    _tcsncpy_s(m_szPluginText, sizeof(m_szPluginText) / sizeof(m_szPluginText[0]), _T("[!output root] Plugin"), sizeof(m_szPluginText) / sizeof(m_szPluginText[0]));
[!endif]
[!if LISTENTOEVENTS]
    m_dwAdviseCookie = 0;
[!endif]
...
HRESULT C[!output Safe_root]::Create(HWND hwndParent, HWND *phwndWindow)
{
    ...
    HWND hWnd = m_pPluginWindow->Create(hwndParent, rcPos, _T("[!output root] Plugin"));
    if (NULL == hWnd)
    {
