...
[!if VSNET]
        ...
        lResult = key.QueryValue(kszPrefsTextColor, &dwType, &dwValue, &uLength);
[!else]
        lResult = key.QueryValue(dwValue, kszPrefsTextColor);
[!endif]
        if (ERROR_SUCCESS == lResult)
        {
...
STDMETHODIMP C[!output Safe_root]::Read( LPCOLESTR pszPropName, VARIANT *pVar, IErrorLog *pErrorLog )
{
    HRESULT hr = S_OK;
    ...
    if ( 0 == _wcsicmp( pszPropName, L"IconStreams" ) )
    {
...
STDMETHODIMP C[!output Safe_root]::Write( LPCOLESTR pszPropName, VARIANT *pVar )
{
...
STDMETHODIMP C[!output Safe_root]::LoadResourceImage( IStream **ppStream )
{
    ...
    *ppStream = NULL;
    ...
    {
        // Find the resource.
        hRsrc = ::FindResource( hInst, MAKEINTRESOURCE( IDR_ICON ), TEXT( "JPG" ) );
        if ( NULL == hRsrc )
        {
            hr = E_HANDLE;
        }
