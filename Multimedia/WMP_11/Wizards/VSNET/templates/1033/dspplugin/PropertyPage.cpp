...
STDMETHODIMP C[!output Safe_root]PropPage::Apply(void)
{
    ...
    GetDlgItemText(IDC_SCALEFACTOR, szStr, sizeof(szStr) / sizeof(szStr[0]));
    _stscanf_s(szStr, _T("%lf"), &fScaleFactor);	

    // make sure scale factor is valid
    if ((fScaleFactor < 0.0) ||
        (fScaleFactor > 1.0))
    {
        if (::LoadString(_Module.GetResourceInstance(), IDS_SCALERANGEERROR, szStr, sizeof(szStr) / sizeof(szStr[0])))
        {
            MessageBox(szStr);
        }
...
[!if VSNET]
        ...
        lResult = key.SetValue(kszPrefsScaleFactor, REG_DWORD, &dwValue, sizeof(dwValue));
[!else]
        lResult = key.SetValue((DWORD) (fScaleFactor * 65536), kszPrefsScaleFactor );
[!endif]
    }
...
[!endif]
    ...
    _stprintf_s(szStr, _T("%0.2f"), fScaleFactor);	
    SetDlgItemText(IDC_SCALEFACTOR, szStr);
