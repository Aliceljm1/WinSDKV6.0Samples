...
{
    *pfAllowPlay = (::MessageBox(hwnd, _T("Allow user to play file?"), _T("Allow Play"), MB_YESNO) == IDYES) ? TRUE : FALSE;
    return S_OK;
}
...
{
    HRESULT hr = S_OK;
    ::MessageBox(hwnd, _T("Starting background processing"), _T("[!output FRIENDLYNAME]"), 0);
    return hr;
}
            ...
            for (long i = 0; i < lNumberItems; i++)
            {
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    if (SUCCEEDED(hr))
                    {
                        ...
                        int iIndex = ::SendMessage(hwndControl, LB_ADDSTRING, 0, reinterpret_cast<LPARAM>(W2A(static_cast<WCHAR*>(cbstr))));
