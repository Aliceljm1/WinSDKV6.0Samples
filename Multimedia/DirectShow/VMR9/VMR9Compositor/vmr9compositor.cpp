...
#pragma message("To build this sample, you must install the DirectX 9 SDK and set the DXSDK_DIR environment variable to the install path.")
...
std::vector<_bstr_t> GetMoviePath()
{
    ...
    if (GetOpenFileName (&ofn))  // user specified a file
    {
        ...
        while( fileName && *fileName ) {
            r.resize( r.size() + 1 );
            r.at( r.size() - 1 ) = path + TEXT("\\") + fileName;
...
HRESULT StartGraph(HWND window)
{
    ...
    if( ! paths.size() )
    {
		MessageBox(NULL, TEXT( "No files selected, not rendering" ),NULL,MB_OK);
        return E_FAIL;
    }
