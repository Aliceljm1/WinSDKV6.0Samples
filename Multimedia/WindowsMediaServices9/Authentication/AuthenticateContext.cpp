...
)
{
    ...
    do
    {
        if( 0 == response.GetLength() )
        {
            ...
            _snwprintf_s( wszChallenge, sizeof( REALM_NAME ) / sizeof( WCHAR ) + 9, L"realm=\"%s\"", REALM_NAME );
            wszChallenge[ sizeof( REALM_NAME ) / sizeof( WCHAR ) + 8 ] = L'\0';
            CSafeArrayOfBytes challenge( &ChallengeBlob );
            hr = challenge.SetData( ( BYTE* ) wszChallenge, sizeof( WCHAR ) * wcslen( wszChallenge ) );
