...
--*/
{
   TCHAR temp[10];
    ...
    try {
        ...
        _tcsncpy_s(temp, ARRAYSIZE(temp), str, 8);
        temp[8] = 0;
        guid->Data1 = ahextoi(temp);
        _tcsncpy_s(temp, ARRAYSIZE(temp), &str[9], 4);
        temp[4] = 0;
        guid->Data2 = (USHORT) ahextoi(temp);
        _tcsncpy_s(temp, ARRAYSIZE(temp), &str[14], 4);
        temp[4] = 0;
        guid->Data3 = (USHORT) ahextoi(temp);

        for (i=0; i<2; i++) {
            _tcsncpy_s(temp, ARRAYSIZE(temp), &str[19 + (i*2)], 2);
            temp[2] = 0;
            guid->Data4[i] = (UCHAR) ahextoi(temp);
        }
        for (i=2; i<8; i++) {
            _tcsncpy_s(temp, ARRAYSIZE(temp), &str[20 + (i*2)], 2);
            temp[2] = 0;
            guid->Data4[i] = (UCHAR) ahextoi(temp);
        }
    }
