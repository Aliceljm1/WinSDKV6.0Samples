...
--*/
{
    ...
    if (EventListHead == NULL) {
        ...
        if (pMofInfo != NULL){
            nSize = (_tcslen(GUID_TYPE_EVENTTRACE) + 1) * sizeof(TCHAR);
            InsertTailList( EventListHead, &pMofInfo->Entry );
            pMofInfo->strDescription = (LPTSTR)malloc(nSize);
            if( pMofInfo->strDescription != NULL ){
                _tcscpy_s(pMofInfo->strDescription, nSize, GUID_TYPE_EVENTTRACE);
            }

            nSize = (_tcslen(GUID_TYPE_HEADER) + 1) * sizeof(TCHAR);
            pMofInfo->strType = (LPTSTR)malloc(nSize);
            if( pMofInfo->strType != NULL ){
                _tcscpy_s(pMofInfo->strType, nSize, GUID_TYPE_HEADER );
            }
        }
    }
    ...
    if (NULL == pMofInfo){
        ...
        if( pMofInfo != NULL ){
            nSize = (_tcslen(GUID_TYPE_UNKNOWN) + 1) * sizeof(TCHAR);
            pMofInfo->strDescription = (LPTSTR)malloc(nSize);
            if (pMofInfo->strDescription != NULL){
                _tcscpy_s(pMofInfo->strDescription, nSize, GUID_TYPE_UNKNOWN);
            }
            InsertTailList( EventListHead, &pMofInfo->Entry );
        }
    }
...
--*/
{
    ...
    while (Head != Next) {
        ...
        if (NULL != pMofInfo) {
            ...
            nSize = (_tcslen(strType) + 1) * sizeof(TCHAR);
            pItem->strDescription = (LPTSTR)malloc(nSize);
            
            if (NULL == pItem->strDescription) {
                free( pItem );
                return;
            }
            _tcscpy_s(pItem->strDescription, nSize, strType);
...
--*/
{
    ...
    hr = pLocator->ConnectServer(bszNamespace,
                                 NULL, 
                                 NULL, 
                                 NULL, 
                                 0L,
...
--*/
{
    ...
    if (SUCCEEDED(hRes) && pVal.vt == VT_I4) {
        ArraySize = pVal.lVal;
    }
    VariantClear(&pVal);
    return ArraySize;
...
--*/
{
    ...
    if (SUCCEEDED(hRes) && NULL != pVal.bstrVal) {
        wcscpy_s(strFormat, 10, pVal.bstrVal);
    }
    ...
    SysFreeString(bszQualName);
    if (SUCCEEDED(hRes) && NULL != pVal.bstrVal) {
        wcscpy_s(strTermination, 30, pVal.bstrVal);
    }
    ...
    SysFreeString(bszQualName);
    if (SUCCEEDED(hRes) && NULL != pVal.bstrVal) {
        wcscpy_s(strTemp, 30, pVal.bstrVal);
    }
...
--*/
{
    ...
    if (SUCCEEDED(hRes)) {
        ...
        hRes = pQualSet->Get(bszFriendlyName, 
                                ...
                                &pClassName, 
                                0);
        if (SUCCEEDED(hRes) && pClassName.bstrVal != NULL) {
#ifdef UNICODE
            wcscpy_s(strClassName, MAXSTR, pClassName.bstrVal);
#else
            ...
            strcpy_s(strClassName, MAXSTR, TempString);
        ...
        if (pMofInfo != NULL) {
            ULONG nSize = (_tcslen(strClassName) + 1) * sizeof(TCHAR);
            pMofTemplate = pMofInfo;
            pMofLookup = pMofInfo;
            InsertTailList(&ListHead, &pMofInfo->Entry);
            pMofInfo->strDescription = (LPTSTR)malloc(nSize);
            if (NULL != pMofInfo->strDescription) {
                _tcscpy_s(pMofInfo->strDescription, nSize, strClassName);
            }
        }
        else {
            goto cleanup;
        ...
        if (SUCCEEDED(hRes)) {
            ...
            while(uReturnedSub == 1){
                ...
                if (SUCCEEDED(hRes) && uReturnedSub == 1) {
                    if (pQualSet) {
                        pQualSet->Release();
                        pQualSet = NULL;
                    }
                    ...
                    if (SUCCEEDED(hRes)) {
                        ...
                        if (pTypeVal.vt & VT_ARRAY) {   // EventType is an array
                            ...
                            hRes = pQualSet->Get(bszEventTypeName, 
                                                    ...
                                                    &pTypeNameVal, 
                                                    0);
                            if (SUCCEEDED(hRes) && (pTypeNameVal.vt & VT_ARRAY)) {
                                ...
                                for (lCount = lTypeLower; lCount <= lTypeUpper; lCount++) { 
                                    ...
                                    if (pMofInfo != NULL) {
                                        ...
                                        if (pMofTemplate != NULL && pMofTemplate->strDescription != NULL) {
                                            ULONG nSize = (_tcslen(pMofTemplate->strDescription) + 1) * sizeof(TCHAR);
                                            pMofInfo->strDescription = (LPTSTR)malloc(nSize);
                                            if (pMofInfo->strDescription != NULL) {
                                                _tcscpy_s(pMofInfo->strDescription, nSize, pMofTemplate->strDescription);
                                            }
                                        }
                                        if (nType == nEventType) {
                                            // Type matched
                                        ...
                                        if (TypeNameArray != NULL) {
                                            if ((lCount >= lTypeNameLower) && (lCount <= lTypeNameUpper)) {
                                                ULONG nSize = (wcslen((LPWSTR)pTypeNameData[lCount]) + 1) * sizeof(TCHAR);
                                                pMofInfo->strType = (LPTSTR)malloc(nSize);
                                                if (pMofInfo->strType != NULL){
#ifdef UNICODE
                                                    wcscpy_s(pMofInfo->strType, nSize, (LPWSTR)(pTypeNameData[lCount]));
#else
                                                    WideCharToMultiByte(CP_ACP,
                                                                        0,
                                                    ...
                                                    strcpy_s(pMofInfo->strType, nSize, TempString);
#endif
                                                }
                                            }
                                        }
                            ...
                            if (SUCCEEDED(hRes)) {
#ifdef UNICODE
                                wcscpy_s(strType, MAXSTR, pTypeNameVal.bstrVal);
#else
                                WideCharToMultiByte(CP_ACP,
                                                    0,
                                ...
                                strcpy_s(strType, MAXSTR, TempString);
                            ...
                            if (pMofInfo != NULL) {
                                ...
                                pMofInfo->strType = (LPTSTR)malloc(nSize);
                                if (pMofInfo->strType != NULL){
                                    _tcscpy_s(pMofInfo->strType, nSize, strType);
                                }
                            }
                        }
...
#ifdef UNICODE
    GuidToString(strTargetGuid, MAXSTR, &Guid);
#else
    CHAR TempString[MAXSTR];
    GuidToString(TempString, MAXSTR, &Guid);
                            ...
                            if (SUCCEEDED(hRes) && uReturnedSub == 1) {
                                ...
                                if (SUCCEEDED(hRes)) {
                                    ...
                                    hRes = pQualSet->Get(bszGuid, 
                                                         ...
                                                         0);
                                    if (SUCCEEDED(hRes)) {
                                        wcscpy_s(strGuid, MAXSTR, (LPWSTR)V_BSTR(&pGuidVal));
                                        VariantClear (&pGuidVal);

                                        if (!wcsstr(strGuid, L"{")) {
                                            swprintf_s(strGuid, MAXSTR, L"{%s}", strGuid);
                                        }
