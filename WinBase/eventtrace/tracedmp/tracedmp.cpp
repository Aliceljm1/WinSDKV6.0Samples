...
--*/
{
    ...
    _tcscpy_s(DumpFileName, MAXSTR, DUMP_FILE_NAME);
    _tcscpy_s(SummaryFileName, MAXSTR, SUMMARY_FILE_NAME);
    ...
    while (--argc > 0) {
        ...
        if (**targv == '-' || **targv == '/') {  // argument found
                                       ...
                                       || targv[0][1] == '?')
            {
                ...
                _tcscpy_s(EvmFile[LogFileCount]->LoggerName, MAXSTR, LoggerName);
                ...
                if (argc > 1) {
                    if (targv[1][0] != '-' && targv[1][0] != '/') {
                        ...
                        _tsplitpath_s(DumpFileName, drive, 10, path, MAXSTR, file, MAXSTR, ext, MAXSTR);
                        _tcscpy_s(ext, MAXSTR, _T("csv"));
                        _tmakepath_s(DumpFileName, MAXSTR, drive, path, file, ext);
                        _tcscpy_s(ext, MAXSTR, _T("txt"));  
                        _tmakepath_s(SummaryFileName, MAXSTR, drive, path, file, ext);
                    }
                }
            }
        }
    ...
    if (!fSummaryOnly)
    {
        if (_tfopen_s(&DumpFile, DumpFileName, _T("w")) != 0) {
            Status = ERROR_INVALID_PARAMETER;
            _tprintf(_T("DumpFile is NULL\n"));
            goto cleanup;
        }
    }
    if (_tfopen_s(&SummaryFile, SummaryFileName, _T("w")) != 0) {
        Status = ERROR_INVALID_PARAMETER;
...
--*/
{
    ...
    if ( pMofInfo->strDescription != NULL ){
        ...
        GuidToString(strGuid, MAXSTR, &pMofInfo->Guid );
                ...
                while (* src != '\0'){
                    if (* src == '\n'){
                        ...
                        if (inQ){
                            ...
                            sprintf_s(strCount, 32, "{%dx}", lineCount);
                            cpy = & strCount[0];
                            while (* cpy != '\0'){
                                * dest ++ = * cpy ++;
                            }
        ...
        case ItemSid:       // SID 
        {
            ...
            else
            {
                               ...
                               & Se))
                {
                    ...
                    swprintf_s(pFullName, 256, L"\\\\%s\\%s", Domain, UserName);
...
--*/
{
    _stprintf_s(s, count, _T("{%08x-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x}"),
...
--*/
{
    ...
    while (Head != Next) {
        ...
        if (pMofInfo->EventCount > 0) {
            GuidToString(&s[0], 256, &pMofInfo->Guid);
            str = s;
            if( pMofInfo->strDescription != NULL ){
                _tcscpy_s(wstr, 256, pMofInfo->strDescription);
            }
