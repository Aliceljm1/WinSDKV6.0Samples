...
a. A client machine boots into a version of Windows Vista that contains the 
   Windows Vista setup binaries. The client boot can be from any media – over 
...
e.g. ‘Images\Image Group (1)\Windows_Vista_ENTERPRISE-NZUUV.wim' 
becomes 
‘\\MyWDSServer\REMINST\Images\Image Group (1)\Windows_Vista_ENTERPRISE-NZUUV.wim’.
...
On Windows Codename “Longhorn” Server, the Windows Deployment Services server
...
accomplished by running the command ‘WDSUTIL.exe /set-server /WDSClientLogging’.
 Logging must be enabled (set to ‘yes’) and the logging level must match that 
...
‘Warnings’ or ‘Info’ in order for the event to be recorded.
