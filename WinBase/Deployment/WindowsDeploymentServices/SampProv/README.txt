...
1. A Windows codename ‘Longhorn’ Server, Windows Server 2003 SP2, or Windows
   Server 2003 SP1 server with the Windows Deployment Services server role 
   installed 
2. A folder shared as ‘REMINST’ 
3. The TFTPD root (e.g. 
   HKLM\System\CurrentControlSet\Services\TFTPD\Parameters\Directory 
   type=REG_SZ on Server 2003 machines) set to the folder shared as ‘REMINST’ 
...
5. Bootmgr.exe, boot.sdi, a .WIM file containing a bootable image 
   ...
   the .WIM file also placed on the network share
...
20 ‘0’s. The name and path of the NBP are relative to the ‘REMINST’ share. 
If a device is not found in the [Devices] section of the .ini file then the 
value specified as the ‘DefaultBootProgram’ will be used.
...
000000000000000000001234567890AB=boot\x86\pxeboot.n12
