...
1. A Windows codename ‘Longhorn’ Server, Windows Server 2003 SP2, or 
   Windows Server 2003 SP1 server with the Windows Deployment Services 
   server role installed 

2. A folder shared as ‘REMINST’ 
3. The TFTPD root (e.g.    
   HKLM\System\CurrentControlSet\Services\TFTPD\Parameters\Directory 
   type=REG_SZ on Server 2003 machines) set to the folder shared as 
   ‘REMINST’ 
4. Network boot programs such as wdsnbp.com, pxeboot.com, pxeboot.n12,
   etc. placed on the share that the client will download and boot into 
5. Bootmgr.exe, boot.sdi, a .WIM file containing a bootable image 
   (generally Windows PE), and an appropriate BCD file for network 
   booting the .WIM file also placed on the network share
