...
HWND WindowsInit ( HINSTANCE a_HInstance )
{
	...
	t_wc.lpszClassName    = reinterpret_cast<LPCTSTR>(t_TemplateCode) ;
 
	ATOM t_winClass = RegisterClass ( &t_wc ) ;

	HWND t_HWnd = CreateWindow (

		reinterpret_cast<LPCTSTR>(t_TemplateCode) ,              // see RegisterClass() call
		reinterpret_cast<LPCTSTR>(t_TemplateCode) ,                      // text for window title bar
