...
{
	wchar_t t_String [ 64 ] ;
	StringCbPrintf(reinterpret_cast<LPTSTR>(t_String), sizeof(t_String), reinterpret_cast<LPTSTR>("%I64u") , a_Uint64 ) ;
...
) 
{
	...
	if ( _wcsicmp ( a_Class , L"ContactInfo" ) == 0 ) 
	{
		...
		BOOL bRes = ConvertStringSecurityDescriptorToSecurityDescriptor //this function is only available on Windows 2000 and above
			(reinterpret_cast<LPCTSTR>("O:BAG:BAD:(A;;0x10000001;;;BA)(A;;0x10000001;;;SY)(A;;0x10000001;;;LA)(A;;0x10000001;;;SY)(A;;0x10000001;;;S-1-5-20)(A;;0x10000001;;;S-1-5-19"),
			SDDL_REVISION_1,
 ...
 */
	...
	if ( SUCCEEDED ( t_Result ) ) 
	{
		...
		if ( m_ComputerName ) 
		{
			...
			if (! GetComputerName ( reinterpret_cast<LPTSTR>(m_ComputerName) , & t_Length ))
			{
