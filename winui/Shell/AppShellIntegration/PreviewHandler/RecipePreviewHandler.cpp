// RecipePreviewHandler.cpp : Implementation of CRecipePreviewHandler
...
void CRecipePreviewHandler::FinalRelease()
{ 
...
HRESULT CRecipePreviewHandler::SetWindow(HWND hwnd, const RECT *prc)
{
    if (hwnd != NULL && prc != NULL)
    {
        ...
        if (_hwndPreview != NULL)
        {
            ...
            SetWindowPos(_hwndPreview, 
                         NULL, 
                         _rcParent.left, 
                         _rcParent.top, 
                         RECTWIDTH(_rcParent), 
                         RECTHEIGHT(_rcParent),
...
HRESULT CRecipePreviewHandler::TranslateAccelerator(MSG *pmsg)
{
    ...
    if (_pFrame != NULL)
    {
        ...
        //
        hr = _pFrame->TranslateAccelerator(pmsg);
    }
...
HRESULT CRecipePreviewHandler::DoPreview()
{
    ...
    if (_hwndPreview == NULL && _pStream != NULL) // cannot call more than once (Unload should be called before another DoPreview)
    {
...
//
HRESULT CRecipePreviewHandler::SetSite(IUnknown *pSite) 
{ 
...
//
HRESULT CRecipePreviewHandler::_CreatePreviewWindow(BSTR bstrRtf)
{
    HRESULT hr = E_FAIL;
    ...
    else
    {
        ...
        _hwndPreview = CreateWindowExW(0, 
                                       MSFTEDIT_CLASS, 
                                       NULL, 
                                       WS_CHILD | WS_VSCROLL | WS_VISIBLE | ES_MULTILINE | WS_BORDER | ES_READONLY,  // Always create read-only
                                       _rcParent.left, 
                                       _rcParent.top, 
                                       RECTWIDTH(_rcParent), 
                                       RECTHEIGHT(_rcParent),
        ...
        else
        {
            ...
            ULONG cchRtf = 1 + (ULONG) wcslen(bstrRtf);
            LPSTR pszRtf = (LPSTR) CoTaskMemAlloc(cchRtf);

            if (pszRtf)
            {
                WideCharToMultiByte(CP_ACP, 0, bstrRtf, cchRtf, pszRtf, cchRtf, NULL, NULL);
            
                SETTEXTEX st = { ST_DEFAULT, CP_ACP };
                
                SendMessage(_hwndPreview, 
                            EM_SETTEXTEX, 
                            (WPARAM) &st, 
                            (LPARAM) pszRtf);
...
HRESULT CRecipePreviewHandler::_CreateStyleSheetNode()
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        else
        {
            ...
            if (SUCCEEDED(hr))
            {
                hr = pDomDoc->QueryInterface(IID_PPV_ARGS(&_pStyleSheetNode));
            }
