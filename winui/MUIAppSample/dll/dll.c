...
HRESULT STDMETHODCALLTYPE Site_OnPosRectChange(IOleInPlaceSite FAR* This, LPCRECT lprcPosRect)
{
    ...
    HWND hwnd = ((_IOleInPlaceSiteEx FAR*)This)->frame->window;
    IOleObject* browserObject = *((IOleObject**)GetWindowLong(hwnd, GWLP_USERDATA));
    if(browserObject != NULL && !browserObject->lpVtbl->QueryInterface(browserObject, &IID_IOleInPlaceObject, (void**)&inplace)) {
...
void WINAPI UnEmbedBrowserObject(HWND hwnd)
{
    ...
    if ((browserHandle = (IOleObject **)GetWindowLong(hwnd, GWLP_USERDATA)))
    {
